...
const giveawayBlacklist = new Set();
...
function saveData() {
  ...
  const data = {
    ...
    giveawayBlacklist: [...giveawayBlacklist],
...
function loadData() {
  ...
  if (data.giveawayBlacklist) {
    ...
    for (const id of data.giveawayBlacklist) {
      giveawayBlacklist.add(id);
    }
  }
 ...
 if (cmd === "staffstats") {
  ...
  if (cmd === "gblacklist") {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `<@${user.id}> has been blacklisted from giveaways`
      )
      .setTimestamp();
